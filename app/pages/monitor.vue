  ...
  <div class="flex-1 min-h-0 overflow-hidden flex flex-col px-3 sm:px-6 py-3 gap-3">
    ...
    <header class="glass shadow-island rounded-3xl px-4 sm:px-6 py-3 flex-shrink-0 flex items-center gap-4 flex-wrap">
      <div class="min-w-0 flex-1">
        <p class="text-[10px] font-bold uppercase tracking-widest text-primary leading-none">{{ t('monitor.eyebrow') }}</p>
        <h2 class="text-xl sm:text-2xl font-black tracking-tighter title-gradient leading-tight mt-0.5">{{ t('monitor.title') }}</h2>
      </div>
      ...
      <button v-if="finishedCount" type="button" class="press h-9 px-4 rounded-full glass text-sm font-semibold text-gray-700 hover:bg-white flex-shrink-0" @click="clearFinished">
        <i class="bi bi-trash3 me-1" />{{ t('monitor.clearFinished') }}
      </button>
    </header>
    ...
    <section class="flex-1 min-h-0 overflow-y-auto pr-1 pb-4">
      <div v-if="!runs.length" class="h-full flex flex-col items-center justify-center text-center text-gray-400">
        ...
        <p class="text-sm font-semibold text-gray-600 mt-3">{{ t('monitor.emptyTitle') }}</p>
        <p class="text-xs text-gray-400 mt-1">{{ t('monitor.emptyHint') }}</p>
        <NuxtLink to="/workflows" class="mt-4 press inline-flex items-center gap-2 h-10 px-5 rounded-full bg-primary text-white text-sm font-semibold shadow-pill hover:bg-primary-dark"><i class="bi bi-diagram-3" />{{ t('monitor.goToWorkflows') }}</NuxtLink>
      </div>
        ...
        <div v-for="r in runs" :key="r.id" class="glass shadow-card rounded-3xl p-3.5 flex items-start gap-3">
          ...
          <div class="min-w-0 flex-1">
            ...
            <div class="text-[11.5px] text-gray-500 truncate mt-0.5">
              <i class="bi bi-dot" />{{ lastMsgText(r) }}
            </div>
            <!-- preview kết quả nếu có -->
            <div v-if="thumb(r)" class="mt-2">
              <img v-if="thumb(r).kind === 'image'" :src="msrc(thumb(r).url)" class="h-16 rounded-lg border border-white/70 object-cover" alt="" />
              <video v-else-if="thumb(r).kind === 'video'" :src="msrc(thumb(r).url)" class="h-16 rounded-lg border border-white/70" muted />
            </div>
            <div class="flex items-center gap-3 mt-2 text-[10.5px] text-gray-400">
              <span><i class="bi bi-clock me-1" />{{ fmtTime(r.created_at) }}</span>
              <span><i class="bi bi-list-task me-1" />{{ t('monitor.steps', { n: (r.events || []).length }) }}</span>
              <NuxtLink v-if="r.workflow_id" :to="`/workflows/${r.workflow_id}`" class="text-primary font-semibold hover:underline ml-auto">{{ t('monitor.open') }}</NuxtLink>
              <button type="button" class="press text-rose-400 hover:text-rose-600" :title="t('monitor.delete')" @click="removeRun(r.id)"><i class="bi bi-x-lg" /></button>
            </div>
          </div>
        </div>
      </div>
...
const wfName = (id) => wfMap.value[id] || t('monitor.deletedWorkflow')
const lastMsgText = (r) => (r.events || [])[(r.events || []).length - 1]?.msg || t('monitor.waiting')
function fmtTime(iso) {
  if (!iso) return ''
  const d = new Date(iso); const diff = (Date.now() - d.getTime()) / 1000
  if (diff < 60) return t('monitor.secAgo', { n: Math.max(0, Math.round(diff)) })
  if (diff < 3600) return t('monitor.minAgo', { n: Math.round(diff / 60) })
...
const ST = computed(() => ({
  running: { label: t('monitor.stRunning'), icon: 'bi-arrow-repeat', bg: 'bg-amber-100 text-amber-700', pill: 'bg-amber-100 text-amber-700' },
  queued:  { label: t('monitor.stQueued'), icon: 'bi-hourglass-split', bg: 'bg-blue-50 text-primary', pill: 'bg-blue-100 text-primary' },
  success: { label: t('monitor.stSuccess'), icon: 'bi-check2-circle', bg: 'bg-emerald-100 text-emerald-700', pill: 'bg-emerald-100 text-emerald-700' },
  error:   { label: t('monitor.stError'), icon: 'bi-x-circle', bg: 'bg-rose-100 text-rose-700', pill: 'bg-rose-100 text-rose-700' }
}))
const stChip = (s) => ST.value[s] || { label: s || '?', icon: 'bi-question', bg: 'bg-gray-100 text-gray-500', pill: 'bg-gray-100 text-gray-500' }

const stats = computed(() => {
  const c = { running: 0, queued: 0, success: 0, error: 0 }
  for (const r of runs.value) c[r.status] = (c[r.status] || 0) + 1
  return [
    { label: t('monitor.statRunning'), value: c.running + c.queued, icon: 'bi-arrow-repeat', bg: 'bg-amber-100 text-amber-700' },
    { label: t('monitor.statDone'), value: c.success, icon: 'bi-check2-circle', bg: 'bg-emerald-100 text-emerald-700' },
    { label: t('monitor.statError'), value: c.error, icon: 'bi-x-circle', bg: 'bg-rose-100 text-rose-700' }
  ]
})
